...
### Step 1: Train the Privileged Teacher Model (Go2-velocity-v0)
Train a teacher agent that has full access to the privileged variables and learns optimized motion gaits over the complex generator terrain:
```bash
python scripts/rsl_rl/train.py --task Go2-velocity-v0 --num_envs 4096
```
*Note: Checkpoints will be saved in `logs/rsl_rl/go2_demo`.*

### Step 2: Distill the Teacher into the Student Model (Go2-velocity-v1)
...
```bash
python scripts/rsl_rl/train.py --task Go2-velocity-v1 --num_envs 4096 --load_run <teacher_run_folder> --checkpoint <teacher_model.pt>
```
*Example: `--load_run 2026-06-29_12-00-00_go2_demo --checkpoint model_10000.pt`*

### Step 3: Fine-Tune the Distilled Student Policy
...
python scripts/rsl_rl/play.py --task Go2-velocity-v1 --load_run <student_run_folder> --checkpoint <model_name.pt>
```
